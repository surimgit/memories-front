...
function TableItem({ memoryTest }: TableItemProps){
  const { seqeunce, testDate, measurementTime, gap } = memoryTest;
  ...
  return(
    <div className='tr'>
      <div className='th sequence'>{seqeunce}</div>
      <div className='th test-date'>{testDate}</div>
      <div className='th measurement-time'>{measurementTime} 초</div>
      <div className='th gap'>{gapText}</div>
    </div>
  )
}
...
export default function MemoryTestComplete() {
  ...
  const { currentPage, setCurrentPage,
          ...
          totalSection,
          setTotalList,
          viewList, pageList } = usePagination<MemoryTest>();
    ...
    <div id="memory-test-complete-wrapper">
      ...
      <div className='test-result-container'>
        <div className='test-result-table'>
          ...
          <div className='tr'>
            <div className='td sequence'>순번</div>
            <div className='td test-date'>검사 날짜</div>
            <div className='td measurement-time'>소요 시간</div>
            <div className='td gap'>차이</div>
          </div>
